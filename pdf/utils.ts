...
export const headerDoc = async (
  doc: jsPDF,
  dte: DteFe | DteCcf | DteFse | DteNce | DteNre | DteExp,
...
export const formatNameByTypeDte = (typeDte: string, short: boolean) => {
  switch (typeDte) {
    ...
    case "11":
      return "COMPROBANTE DE FACTURA DE EXPORTACION";
    case "14":
...
export const generateUrl = (dte: DteFe | DteCcf | DteFse | DteNce | DteNre | DteExp) => {
  return (
    $MH_QUERY +
    "?ambiente=" +
...
export const secondHeaderExp = (
  doc: jsPDF,
  dte: DteFe | DteCcf | DteNce | DteNre | DteExp,
  selloInvalidacion = "",
  _contingence: boolean = false,
) => {
  const { receptor, identificacion, respuestaMH, resumen } = dte as DteExp;
  ...
  autoTable(doc, {
    ...
    body: [
      ...
      [
        {
          content: [
            `NOMBRE COMERCIAL: ${receptor.nombreComercial ?? '-'}`,
          ],
        },
        { content: [`CODIGO GENERACION: ${identificacion.codigoGeneracion}`] },
      ],
      [
        { content: [`CODIGO PAIS: ${receptor.codPais}`] },
        { content: [`SELLO RECIBIDO: ${respuestaMH.selloRecibido}`] },
      ],
      [
        { content: [`NOMBRE PAIS: ${receptor.nombrePais ?? "-"}`] },
        { content: [`TELEFONO: ${receptor.telefono ?? "-"}`] },
      ],
      [
        { content: [`CORREO: ${receptor.correo}`] },
        {
          content: [
            `TIPO DE PERSONA: ${returnTypePerson(String(receptor.tipoPersona))}`,
          ],
        },
      ],
       [
        { content: [`TIPO DE DOCUMENTO: ${returnTypeDocument(receptor.tipoDocumento)}`] },
        { content: [`NUMERO DOCUMENTO: ${receptor.numDocumento ?? "-"}`] },
      ],
       [
        { content: [`DIRECCION: ${receptor.complemento}`], colSpan: 2 },
      ],
      selloInvalidacion !== ""
        ? [
            {
    ...
    didParseCell: (data) => {
      ...
      if (
        raw.startsWith("NOMBRE:") ||
        raw.startsWith("NUMERO DE CONTROL:") ||
        raw.startsWith("NOMBRE COMERCIAL:") ||
        raw.startsWith("CODIGO GENERACION:") ||
        raw.startsWith("CODIGO PAIS:") ||
        raw.startsWith("SELLO RECIBIDO:") ||
        raw.startsWith("NUMERO DOCUMENTO:") ||
        raw.startsWith("NOMBRE PAIS:") ||
        raw.startsWith("TELEFONO:") ||
        raw.startsWith("CORREO:") ||
        raw.startsWith("TIPO DE PERSONA:") ||
        raw.startsWith("TIPO DE DOCUMENTO:") ||
        raw.startsWith("DIRECCION:")
      ) {
        data.cell.text = [""];

        if (raw.startsWith("DIRECCION:")) {
          data.cell.styles.minCellHeight = 6;
        }
      }
    },
    ...
    didDrawCell: (data) => {
      ...
      if (raw.startsWith("NUMERO DE CONTROL:")) {
        drawLabel("NUMERO DE CONTROL: ", raw.replace("NUMERO DE CONTROL:", "").trim());
      }

      if (raw.startsWith("NOMBRE COMERCIAL:")) {
        drawLabel("NOMBRE COMERCIAL: ", raw.replace("NOMBRE COMERCIAL:", "").trim());
      }

      if (raw.startsWith("CODIGO GENERACION:")) {
        drawLabel(
          "CODIGO GENERACION:",
          raw.replace("CODIGO GENERACION:", "").trim(),
        );
      }
      if (raw.startsWith("CODIGO PAIS:")) {
        drawLabel("CODIGO PAIS:", raw.replace("CODIGO PAIS:", "").trim());
      }
      if (raw.startsWith("SELLO RECIBIDO:")) {
        drawLabel("SELLO RECIBIDO:", raw.replace("SELLO RECIBIDO:", "").trim());
      }
      if (raw.startsWith("NUMERO DOCUMENTO:")) {
        drawLabel(
          "NUMERO DOCUMENTO:",
      ...
      if (raw.startsWith("NOMBRE PAIS:")) {
        drawLabel("NOMBRE PAIS:", raw.replace("NOMBRE PAIS:", "").trim());
      }
      ...
      if (raw.startsWith("CORREO:")) {
        drawLabel(
          "CORREO:",
          raw.replace("CORREO:", "").trim(),
        );
      }
      if (raw.startsWith("TIPO DE PERSONA:")) {
        drawLabel("TIPO DE PERSONA:", raw.replace("TIPO DE PERSONA:", "").trim());
      }
      if (raw.startsWith("TIPO DE DOCUMENTO:")) {
        drawLabel(
          "TIPO DE DOCUMENTO:",
          raw.replace("TIPO DE DOCUMENTO:", "").trim(),
        );
      }
      if (raw.startsWith("DIRECCION:")) {
        drawLabel(
          "DIRECCION:",
          raw.replace("DIRECCION:", "").trim(),
        );
      }
    },
  });
...
export const returnTypeDocument = (code: string) => {
  const services = new SeedcodeCatalogosMhService();

  const finded = services
    .get022TipoDeDocumentoDeIde()
    .find((type) => type.codigo === code);

  return finded?.valores ?? "OTROS";
};

export const returnTypePerson = (code: string) => {
  const services = new SeedcodeCatalogosMhService();
